...
def check_subscriptions_loop():
    while True:
        ...
        try:
            ...
            conn.close()
            print(f"[Проверка подписок] Участников: {len(participants)}")
            for (uid,) in participants:
                subscribed = check_tg_subscription(uid)
                print(f"[Проверка подписок] user_id={uid} subscribed={subscribed}")
                if not subscribed:
                    conn = get_conn()
                    c = conn.cursor()
                    ...
                    conn.commit()
                    conn.close()
                    print(f"[Проверка подписок] user_id={uid} исключён из конкурса")
                    try:
                        bot.send_message(
                            uid,
                            "⚠️ Вы отписались от Telegram канала и были исключены из конкурса.\n"
                            "Подпишитесь снова и нажмите /start для повторного участия."
                        )
                    except Exception as e:
                        print(f"[Проверка подписок] Не удалось отправить сообщение user_id={uid}: {e}")
        except Exception as e:
            print(f"[Проверка подписок] Ошибка: {e}")
...
def get_user_status(user_id):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT tg_done, kick_done, vk_done FROM users WHERE user_id=%s", (user_id,)
    )
    result = c.fetchone()
    conn.close()
...
def receive_kick_username(message):
    ...
    c = conn.cursor()
    c.execute(
        "UPDATE users SET kick_done=1, kick_username=%s WHERE user_id=%s",
...
def receive_vk_link(message):
    ...
    c = conn.cursor()
    c.execute(
        "UPDATE users SET vk_done=1, vk_link=%s WHERE user_id=%s", (link, user_id)
    )
    conn.commit()
    conn.close()
...
def callback_handler(call):
    ...
    if call.data == "participate":
        ...
        bot.answer_callback_query(call.id)
        markup_vk = types.InlineKeyboardMarkup()
        markup_vk.add(types.InlineKeyboardButton("🔗 Перейти в группу ВКонтакте", url="https://vk.ru/club236731579"))
        bot.send_message(
            call.message.chat.id,
            "🔄 *Репост ВКонтакте*\n\n"
            "1. Перейди в нашу группу ВКонтакте (кнопка ниже)\n"
            "2. Сделай репост нашей записи на свою страницу\n"
            "3. Открой свою запись с репостом\n"
            "4. Скопируй ссылку и отправь её сюда\n\n"
            "📎 Пришли ссылку вида: https://vk.ru/... или https://vk.com/...\n\n"
            "⚠️ Задание засчитается только после отправки ссылки на репост.",
            parse_mode="Markdown",
            reply_markup=markup_vk,
        )
        ...
        if tg_done:
            ...
            c.execute(
                "UPDATE users SET tg_done=0, joined=0 WHERE user_id=%s", (user_id,)
            )
        conn.commit()
        conn.close()
        ...
        if tg_done and kick_done and vk_done:
            conn = get_conn()
            c = conn.cursor()
            c.execute("SELECT pg_advisory_xact_lock(999999)")
            ...
            if not ticket:
                ...
                c.execute(
                    "UPDATE users SET tg_done=1, joined=1, ticket_number=%s WHERE user_id=%s",
                    (ticket, user_id),
                )
            else:
                c.execute(
                    "UPDATE users SET tg_done=1, joined=1 WHERE user_id=%s", (user_id,)
                )
            conn.commit()
            conn.close()
            ...
            markup.add(
                types.InlineKeyboardButton(
                    "🎫 Получить билет", callback_data="get_ticket"
                )
            )
        ...
        for uid, uname in all_joined:
            if check_tg_subscription(uid):
                ...
                c.execute(
                    "UPDATE users SET tg_done=0, joined=0 WHERE user_id=%s", (uid,)
                )
...
if __name__ == "__main__":
    init_db()
    setup_commands()
    threading.Thread(target=check_subscriptions_loop, daemon=True).start()
